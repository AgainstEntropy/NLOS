...
def check_data(check_root='/mnt/cfs/wangyh/blender/blank_wall/output_png',
               result_dir='/mnt/cfs/wangyh/blender/blank_wall/datasets'):
    ...
    completed_dirs = []
    action_samples = dict.fromkeys(actions, 0)
            ...
            for action in actions:
                for character in characters:
                    for lt in light_types:
                        for le in light_energy_modes:
                            for lp in light_position_modes:

                                dir_name = f'{action}/{action}_{character}_lt{lt}_le{le}_lp{lp}_{scene}-{r}'
    ...
    result_file = os.path.join(result_dir, 'statistic.txt')
